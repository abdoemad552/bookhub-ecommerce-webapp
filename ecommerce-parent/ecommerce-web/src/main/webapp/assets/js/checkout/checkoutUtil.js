...
export function appendNewAddressCard() {
    ...
    card.querySelector('input[type="radio"]').addEventListener('click', showNewAddressPanel);
    grid.appendChild(card);
}

export function showNewAddressPanel() {
    const panel = $('new-addr-panel');
    if (!panel) return;
    if (panel.classList.contains('open')) hideNewAddressPanel();
...
export function openPopup(address) {
    const overlay = $('address-popup-overlay');
...
export function closePopup() {
    $('address-popup-overlay').classList.remove('open');
...
export function getAddressIcon(type) {
    switch (type) {
        case 'Home':
            return `<svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" viewBox="0 0 16 16">
