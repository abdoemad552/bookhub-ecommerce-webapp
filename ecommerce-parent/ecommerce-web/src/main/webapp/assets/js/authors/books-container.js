...
const STORAGE_KEY = 'authorBooksGridLayout';

export class AuthorBooksContainer {
    ...
    init() {
        this._$container = $("#author-books-container");

        if (!this._$container.length) {
            console.error('AuthorBooksContainer: #author-books-container not found in DOM.');
            return;
        }
    ...
    _applyLayoutToGrid() {
        ...
        const $grid = this._$container.find('#author-books-grid');
        if ($grid.length) {
    ...
    _fadeGrid(callback) {
        ...
        setTimeout(() => {
            callback();
            const $next = this._$container.find('#author-books-grid');
    ...
    _fetch(page, debounceTime) {
        ...
        const authorId = this._$container.data("authorId");

        this._activeRequest = $.ajax({
            url: `${getContextPath()}/author/books`,
            method: "GET",
            data: { authorId, page, size: this._pageSize },
            dataType: "html"
        })
            .done((content) => {
                setTimeout(() => {
                    this._fadeOut().then(() => {
                        ...
                        const total = parseInt(
                            this._$container
                                .find('#author-books-grid, #no-books-placeholder')
            ...
            .fail((jqXHR) => {
                console.error('AuthorBooksContainer fetch failed:', jqXHR.responseText);
            });
    }
    ...
    _bindEvents() {
        ...
        $('#grid-btn-1').on('click.authorBooks', () => this._setLayout(1));
        $('#grid-btn-2').on('click.authorBooks', () => this._setLayout(2));
        $('#grid-btn-3').on('click.authorBooks', () => this._setLayout(3));

        // Pagination is still inside the dynamic container — keep delegated.
        this._$container.on('click.authorBooks', '.pagination-page:not([disabled])', (e) => {
