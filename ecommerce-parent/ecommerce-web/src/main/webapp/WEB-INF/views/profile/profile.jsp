    ...
    <main class="max-w-6xl mx-auto px-4 sm:px-6 py-8 profile-layout">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <div class="lg:col-span-3">
                ...
                <%-- ── Wishlist ── --%>
                <div id="wishlist-info" class="profile-panel hidden bg-card text-card-foreground rounded-2xl border shadow-sm p-6 lg:p-7">
                    <div class="panel-header">
                        <div>
                ...
                <%-- ── Addresses ── --%>
                <div id="addresses-info" class="profile-panel hidden bg-card text-card-foreground rounded-2xl border shadow-sm p-6 lg:p-7">
                    <div class="panel-header">
                        <div>
                            <h2 class="text-foreground">Saved Addresses</h2>
                            <p class="text-muted-foreground">Add a new address, or overwrite an existing one by picking the same type.</p>
                        </div>
                    </div>
                                        ...
                                        <div class="address-card">
                                            <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                                <div class="flex flex-col gap-1.5">
                                                    ...
                                                    <p class="text-sm font-semibold text-foreground">${address.street}, Building ${address.buildingNo}</p>
                ...
                <%-- ── Credit Management ── --%>
                <div id="credit-manage-info" class="profile-panel hidden bg-card text-card-foreground rounded-2xl border shadow-sm p-6 lg:p-7">
                    <div class="panel-header">
                        <div>
                    ...
                    <%-- ★ Visual credit card — exact replica of checkout pay-card ★ --%>
                    <div class="credit-card-scene">
                        <div class="credit-card-visual">
                            <%-- Shine layer --%>
                            <div class="credit-card-shine" aria-hidden="true"></div>

                            <%-- Top row: SVG chip + brand --%>
                            <div class="credit-card-top-row">
                                <div class="credit-card-chip-wrap" aria-hidden="true">
                                    <%-- Identical SVG chip to checkout.jsp --%>
                                ...
                                <span class="credit-card-brand-label">BookHub</span>
                            </div>

                            <%-- Card number placeholder --%>
                            <div class="credit-card-number-row">•••• •••• •••• ----</div>

                            <%-- Bottom row: holder + balance --%>
                            <div class="credit-card-bottom-row">
                                <div class="credit-card-meta-block">
                                    <span class="credit-card-meta-label">Card Holder</span>
                                    <span class="credit-card-meta-value">${requestScope.profileUser.username}</span>
                                </div>
                                <div class="credit-card-meta-block" style="text-align:right">
                                    <span class="credit-card-meta-label">Available Balance</span>
                                    <span class="credit-card-meta-value">${requestScope.profileUser.creditLimit} EGP</span>
                                </div>
                            </div>

                            <%-- Usage bar --%>
                            <div class="credit-card-bar-track">
                                <div class="credit-card-bar-fill" style="width:0%"></div>
                            </div>
                        </div>
                    </div>
...
<script type="module">
    ...
    function showProfilePanel(tabId) {
        ...
        profileTabs.forEach(btn => {
            ...
            /* tab-active drives the CSS — no inline Tailwind colour classes needed */
    ...
    function validatePasswordFields() {
        ...
        if (!cur && !nw && !conf) { if (passwordValidationMsg) { passwordValidationMsg.textContent = ""; passwordValidationMsg.className = "mt-3 text-sm text-muted-foreground"; } return true; }
        let msg = "";
        if (!cur)           { currentPasswordInput.setCustomValidity("Required"); msg = msg || "Enter your current password to change it."; }
        if (!nw)            { newPasswordInput.setCustomValidity("Required");     msg = msg || "Enter a new password."; }
        else if (nw.length < 8) { newPasswordInput.setCustomValidity("Too short"); msg = msg || "New password must be at least 8 characters."; }
        else if (cur === nw)    { newPasswordInput.setCustomValidity("Same");      msg = msg || "New password must differ from the current password."; }
        if (!conf)          { confirmNewPasswordInput.setCustomValidity("Required"); msg = msg || "Please retype the new password."; }
        else if (nw && conf !== nw) { confirmNewPasswordInput.setCustomValidity("Mismatch"); msg = msg || "Passwords do not match."; }
